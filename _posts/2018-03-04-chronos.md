---
layout: post
title: "paper research(一)：Decimeter-Level Localization with a Single WiFi Access Point"
category: paper research note
---

# Decimeter-Level Localization with a Single WiFi Access Point

**URL:** https://www.usenix.org/node/194923

**Conference:** the USENIX Symposium on Networked Systems Design and Implementation (NSDI),NSDI'16

##  1. Overview

​        chronos是使用一个ap点实现分米级定位的定位系统。主要的工作是解决如何准确的计算出AP和client之间信号的time-of-flight。得到time-of-flight之后然后再乘以光速得到AP和client之间的距离。（time-of-flight以下简称tof）
...
![单个频带计算tof]({{site.url}}/picture/单个频带计算tof.png)
...
wifi信号在2.4Ghz和5Ghz周围存在不连续的带宽。如下图所示。
...
前面我们提到wifi信号在2.4Ghz和5Ghz周围有很多不连续的频带(channel)，扫频就是在这些频带中的一个频带上发送和接收完之后，再换一个频带继续发送和接收。 这样从第一个channel到最后一个channel，我们就拥有了一个很宽的带宽。
...
从上图中可以看出SFO和PDD对正数序号的载波和负数序号的载波分别造成的正向的影响和负向的影响，但是恰好对0号载波没有造成影响，所以我们可以利用0载波进行计算。

**然而**，0号载波是”不存在的“，，所以我们要用插值法补出0号载波的值。
